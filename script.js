...
function initProductTabs() {
    ...
    function openPanel(id, shouldScroll=false) {
        ...
        if (shouldScroll) {
            const panel = document.querySelector(`[data-product-panel="${id}"]`);
            if (panel) panel.scrollIntoView({ behavior:'smooth', block:'start' });
        }
        history.replaceState(null, '', '#' + id);
    }
    tabs.forEach(tab => tab.addEventListener('click', () => openPanel(tab.dataset.productTab, true)));
    const hash = decodeURIComponent(location.hash.replace('#',''));
    const initial = tabs.some(t => t.dataset.productTab === hash) ? hash : 'advanced-ceramics';
    openPanel(initial, false);
    document.querySelectorAll('[data-collapse-products]').forEach(btn => btn.addEventListener('click', () => {
...
function initCredentialTabs() {
    ...
    if (!tabs.length) return;
    function open(cat, shouldScroll=false) {
        ...
        if (shouldScroll) {
            const panel = document.querySelector(`[data-credential-panel="${cat}"]`);
            if (panel) panel.scrollIntoView({ behavior:'smooth', block:'start' });
        }
    }
    tabs.forEach(t => t.addEventListener('click', () => open(t.dataset.credentialTab, true)));
    document.querySelectorAll('[data-collapse-credentials]').forEach(btn => btn.addEventListener('click', () => {
        tabs.forEach(t => t.classList.remove('active'));
        panels.forEach(p => p.classList.remove('active'));
        const selector = document.querySelector('#credentialTabs');
        if (selector) selector.scrollIntoView({ behavior:'smooth', block:'start' });
    }));
    document.querySelectorAll('[data-scroll-target]').forEach(btn => btn.addEventListener('click', () => {
        const target = document.querySelector(btn.dataset.scrollTarget);
    ...
    open(tabs[0].dataset.credentialTab, false);
}
