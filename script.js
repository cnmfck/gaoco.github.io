...
function applyLanguage(lang) {
    ...
    document.querySelectorAll('[data-aria-zh][data-aria-en]').forEach(el => {
        const value = el.getAttribute(lang === 'zh' ? 'data-aria-zh' : 'data-aria-en');
        if (value) el.setAttribute('aria-label', value);
    });
    const pageTitle = document.body.getAttribute(lang === 'zh' ? 'data-title-zh' : 'data-title-en');
...
function initProductTabs() {
    ...
    function openPanel(id) {
        ...
        tabs.forEach(t => t.setAttribute('aria-selected', t.dataset.productTab === id ? 'true' : 'false'));
    }
    tabs.forEach(tab => tab.addEventListener('click', () => {
        const id = tab.dataset.productTab;
        openPanel(id);
        if (id) history.replaceState(null, '', `#${id}`);
        document.querySelector('.product-panels')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }));
    const hash = decodeURIComponent(location.hash.replace('#',''));
    const initial = tabs.some(t => t.dataset.productTab === hash) ? hash : 'precision-gears';
    openPanel(initial);
}
function initStarAssembly() {
    document.querySelectorAll('[data-star-assembly]').forEach(root => {
        ...
        btn.addEventListener('click', () => {
            ...
            btn.setAttribute('aria-expanded', open ? 'true' : 'false');
            const label = btn.querySelector('.exploded-action b') || btn.querySelector('[data-i18n="clickExpandStars"]');
            if (label) label.textContent = open
                ? (currentLang === 'zh' ? '点击收起零部件' : 'Collapse components')
                : (currentLang === 'zh' ? '点击展开零部件' : 'Explore components');
        });
    });
}
...
function initLiveTiles() {
    ...
    document.querySelectorAll('[data-live-tile]').forEach(tile => {
        const images = Array.from(tile.querySelectorAll('.live-tile-images img'));
        if (images.length < 2) return;
        let current = Math.max(0, images.findIndex(img => img.classList.contains('is-current')));
        const interval = Number(tile.dataset.interval) || 5000;
        const delay = Number(tile.dataset.delay) || 0;
        const advance = () => {
            ...
            window.setTimeout(() => previous.classList.remove('is-leaving'), 760);
        };
        window.setTimeout(() => {
            advance();
...
function initCredentialTabs() {
    ...
    const hash = decodeURIComponent(location.hash.replace('#','').replace('cred-',''));
    const initial = tabs.some(t => t.dataset.credentialTab === hash) ? hash : 'certification';
    open(initial);
}
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (toggle) toggle.addEventListener('click', () => { applyLanguage(currentLang === 'zh' ? 'en' : 'zh'); });
});
